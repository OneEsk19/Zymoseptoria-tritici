...
Choose 10k as indicated here:
https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0214201

Web based version here:
http://swav.popgenetics.net/

The following fst pipeline is based on this:
...
```{r}
snp_matrix  <- read.table("FST/snp_matrix.txt")
```
...
```{r}
write.csv(snp_matrix, "FST/snp_matrix.csv")
```
