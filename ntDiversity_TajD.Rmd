...
```{r}
pi_all <- read.table("./FST/all_samples.windowed.pi", header = T)

p1 <- ggplot(pi_all, aes(x=PI))+
...
```{r}
pi_pre <- read.table("./FST/pre_samples.windowed.pi", header = T)

p3 <- ggplot(pi_pre, aes(x=PI))+
  geom_histogram(bins = 20, fill="darkslategrey", color="burlywood3")+
  xlab("")+
  ylab("Pre-drought")
```
...
```{r}
pi_post <- read.table("./FST/post_samples.windowed.pi", header = T)

p5 <- ggplot(pi_post, aes(x=PI))+
  geom_histogram(bins = 20, fill="darkslategrey", color="burlywood3")+
  xlab("pi")+
  ylab("Post-drought")
```
...
2) Tajima's D
```{r}
taj_all <- read.table("./FST/all_samples.Tajima.D", header = T)

p2 <- ggplot(taj_all, aes(x=TajimaD))+
...
```{r}
taj_pre <- read.table("./FST/pre_samples.Tajima.D", header = T)

p4 <- ggplot(taj_pre, aes(x=TajimaD))+
  geom_histogram(bins=20, fill="burlywood3", color="darkslategrey")+
  xlab("")+
  ylab("")
```
...
```{r}
taj_post <- read.table("./FST/post_samples.Tajima.D", header = T)

p6 <- ggplot(taj_post, aes(x=TajimaD))+
  geom_histogram(bins=20, fill="burlywood3", color="darkslategrey")+
  xlab("Tajima's D")+
ylab("")
```
